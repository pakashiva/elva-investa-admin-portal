-- Per-investment referral commission rate for admin approval (default 1%).
-- Additive column only; trigger uses investment rate when set.
...
COMMENT ON COLUMN public.investments.referral_rate IS
  'Admin-set referral commission rate for this investment (e.g. 0.01 = 1%). Used when Active reward is created.';

-- Use per-investment rate (fallback to global settings, then 1%).
...
DECLARE
  v_result JSON;
  v_default_referral NUMERIC(8, 6);
  v_referral_tds NUMERIC(8, 6);
BEGIN
  SELECT COALESCE(referral_rate, 0.01), COALESCE(tds_rate, 0.02)
...
DROP FUNCTION IF EXISTS public.admin_update_investment_terms(UUID, NUMERIC, NUMERIC, INTEGER);
DROP FUNCTION IF EXISTS public.admin_update_investment_terms(UUID, NUMERIC, NUMERIC, INTEGER, NUMERIC);
