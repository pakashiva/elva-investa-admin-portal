-- Identity fields needed by the loan agreement document:
--   customer Aadhaar (kyc_documents.aadhaar_number)
--   nominee Aadhaar / PAN / mobile (nominees)
-- Additive columns only; create + edit RPCs now carry these through.

ALTER TABLE public.nominees
  ADD COLUMN IF NOT EXISTS nominee_pan TEXT,
  ADD COLUMN IF NOT EXISTS nominee_mobile TEXT;

COMMENT ON COLUMN public.nominees.nominee_pan IS
  'Nominee PAN, printed on the loan agreement.';
COMMENT ON COLUMN public.nominees.nominee_mobile IS
  'Nominee contact number, printed on the loan agreement.';

-- ---------------------------------------------------------------------------
-- Customer details: expose Aadhaar + nominee for the edit form
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Create customer: Aadhaar + nominee identity
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  INSERT INTO public.nominees (
    ...
    NULLIF(v_nominee_pan, ''),
    NULLIF(v_nominee_mobile, '')
  )
...
-- ---------------------------------------------------------------------------
-- Update profile: Aadhaar + nominee identity (NULL leaves a value unchanged)
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.admin_update_customer_profile(
  ...
  IF v_nominee_name <> ''
    ...
    ELSE
      ...
      SET
        ...
        nominee_pan = COALESCE(NULLIF(v_nominee_pan, ''), nominee_pan),
        nominee_mobile = COALESCE(NULLIF(v_nominee_mobile, ''), nominee_mobile)
...
REVOKE ALL ON FUNCTION public.admin_update_customer_profile(
  UUID, TEXT, TEXT, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, TEXT,
  TEXT, TEXT, TEXT, TEXT, TEXT, TEXT
) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.admin_update_customer_profile(
  UUID, TEXT, TEXT, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, TEXT,
  TEXT, TEXT, TEXT, TEXT, TEXT, TEXT
) TO anon, authenticated;
